...
impl AABB {
    ...
    pub fn new(min: &Point3D, max: &Point3D) -> Result<AABB, JsValue> {
        let inner = core::AABB::new(min.inner, max.inner).map_err(to_js_error)?;
        Ok(AABB { inner })
    }
    ...
    pub fn from_points(points: Vec<Point3D>) -> Result<AABB, JsValue> {
        ...
        let inner = core::AABB::from_points(&core_points).map_err(to_js_error)?;
        Ok(AABB { inner })
    }
    ...
    pub fn expand_by_point(&self, point: &Point3D) -> AABB {
        AABB {
            inner: self.inner.expand_by_point(&point.inner),
        }
    }
    ...
    pub fn expand_by_scalar(&self, amount: f64) -> AABB {
        AABB {
            inner: self.inner.expand_by_scalar(amount),
        }
    }
}
