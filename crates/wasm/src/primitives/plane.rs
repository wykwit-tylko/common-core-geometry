...
impl Plane {
    ...
    pub fn from_point_normal(point: &Point3D, normal: &Vector3D) -> Result<Plane, JsValue> {
        let inner = core::Plane::from_point_normal(&point.inner, &normal.inner)
            .map_err(to_js_error)?;
        Ok(Plane { inner })
    }
    ...
    pub fn from_three_points(p1: &Point3D, p2: &Point3D, p3: &Point3D) -> Result<Plane, JsValue> {
        let inner = core::Plane::from_three_points(&p1.inner, &p2.inner, &p3.inner)
            .map_err(to_js_error)?;
        Ok(Plane { inner })
    }
    ...
    pub fn flip_normal(&self) -> Plane {
        Plane { inner: self.inner.flip_normal() }
    }
