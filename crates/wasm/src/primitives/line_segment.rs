...
impl LineSegment {
    ...
    pub fn new(start: &Point3D, end: &Point3D) -> Result<LineSegment, JsValue> {
        let inner = core::LineSegment::new(start.inner, end.inner)
            .map_err(to_js_error)?;
    ...
    pub fn start(&self) -> Point3D {
        Point3D { inner: self.inner.start }
    }

    #[wasm_bindgen(getter)]
    pub fn end(&self) -> Point3D {
        Point3D { inner: self.inner.end }
    }
